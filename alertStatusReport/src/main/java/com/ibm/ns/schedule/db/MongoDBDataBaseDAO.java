...
public class MongoDBDataBaseDAO {
	...
	@Value("#{prop.MONGODB_HOST}")
	public void setMONGODB_HOST(String mONGO_HOST) {
		MONGODB_HOST = mONGO_HOST;
	}

	@Value("#{prop.MONGODB_PORT}")
	public void setMONGODB_PORT(String mONGO_PORT) {
		MONGODB_PORT = mONGO_PORT;
	}

	@Value("#{prop.MONGODB_NAME}")
	public void setMONGODB_NAME(String mONGO_NAME) {
	 ...
	 * @return
	 */
	private MongoDatabase getMongoDBInstance() {
		if(mongoDatabase==null) {
			MongoClient mongoClient = new MongoClient( MONGODB_HOST , Integer.parseInt(MONGODB_PORT) );
		       
	        // 连接到数据库
	        mongoDatabase = mongoClient.getDatabase(MONGODB_NAME); 
		}
	 ...
	 * @param collection
	 */
	void writeDocument(Document doc, String collection) {
